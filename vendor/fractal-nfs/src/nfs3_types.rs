...
impl Ftype3 {
    pub fn from_mode(mode: u32) -> Self {
        match mode & u32::from(libc::S_IFMT) {
            x if x == u32::from(libc::S_IFREG) => Ftype3::Reg,
            x if x == u32::from(libc::S_IFDIR) => Ftype3::Dir,
            x if x == u32::from(libc::S_IFBLK) => Ftype3::Blk,
            x if x == u32::from(libc::S_IFCHR) => Ftype3::Chr,
            x if x == u32::from(libc::S_IFLNK) => Ftype3::Lnk,
            x if x == u32::from(libc::S_IFSOCK) => Ftype3::Sock,
            x if x == u32::from(libc::S_IFIFO) => Ftype3::Fifo,
            _ => Ftype3::Reg,
        }
    }
}
